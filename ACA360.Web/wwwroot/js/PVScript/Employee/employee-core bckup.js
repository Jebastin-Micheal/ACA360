    ...
    // CASE B — Flat form
        ...
        $.each(formData, function () {
            ...
            if (this.name === "State") {
                ...
                payload["State"] = stateText ? stateText.trim() : null;
                ...
                payload["Country"] = countryText ? countryText.trim() : null;
            }
            else {
                payload[this.name] = val;
            }
...
window.updateNavBar = function () {
    ...
    var $prev    = $('#btnEmpPrev');
    var $next    = $('#btnEmpNext');
    var $counter = $('#empNavCounter');   // middle btn span + card-header badge
...
// ── Double-click on ANY field inside a known section ─────────────────────────
// We delegate broadly — fields are disabled by default so we cannot filter
// by :enabled. The selector targets the elements regardless of disabled state.
$(document).on('dblclick', '[id^="section_"] input, [id^="section_"] select, [id^="section_"] textarea', function () {
    var $field  = $(this);

    // Skip readonly fields (SSN mask, hidden inputs, etc.)
    if ($field.is('[readonly]') || $field.is('[type="hidden"]')) return;

    var section = _findSection($field);
    if (!section) return;

    // If already in edit mode, just focus the clicked field
    ...
    window.toggleSection(section.id, true);

    // Focus the exact field that was double-clicked after inputs are enabled
    setTimeout(function () { $field.trigger('focus'); }, 80);
});
