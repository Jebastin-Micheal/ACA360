...
async function setSession(key, value) {
    ...
    return fetch('/Account/SetSessionData', {
        method: 'POST',
        headers: {
...
//    const res = await fetch(`/Account/GetSessionValue?key=${key}`);
...
/*Use this function to show loader inside partial views or any specific division*/
function partialloader(targetClass, message = "Loading, please wait...") {
    if (!targetClass) return;

    const $target = $(`div.${targetClass}`);
    if ($target.length === 0) return;

    $target.block({
        message: `<br><img src="/images/busy.gif" /><br/>${message}<br/><br/>`,
        css: {
...
/*Use this function to hide loader inside partial views or any specific division*/
function partialUnblock(targetClass) {
    if (!targetClass) return;
